...
function startGame() {
	if(audio == null || startMenuObj == null || timeObj == null || hint == null ||
	restObj == null || failObj == null || resultObj == null)
	return; // 게임을 진행할 수 없음

playsound("/resources/audio/recycle.mp3"); // 시작 사운드 재생
...
document.images[i].src="/resources/img/img1.gif"; // 보통 달걀이미지
...
document.images[index].src="/resources/img/img2.gif"; // 숨은 강아지 이미지
i++;
	}
	}
...
function showHint(){
	for(var i=0; i<mark.length; i++) {
	if(mark[i] == true) {
	document.images[i].src="/resources/img/img2.gif"; // 숨은 강아지 달걀 출력
}
	}
...
function hideHint() {
	...
	if(mark[i] == true){
	document.images[i].src="/resources/img/img1.gif";
	}
	}
}
...
playsound("/resources/audio/bi.mp3"); // 남은 시간이 끝나고 사용자가 찾아야 함을 알리는 경고음
hideAnswer(); // 정답 숨기기
}
else if(watingSeconds > 0 && watingSeconds <= 5) // 남은 시간이 1~5초 사이이면 경고음
playsound("/resources/audio/clock.mp3");
}
...
playsound("/resources/audio/chimes.mp3"); // 숨은 강아지 발견의 성공을 알리는 소리 출력

document.images[clickIndex].src="/resources/img/img2.gif"; // 숨은 강아지 달걀 이미지 출력
...
audio.onended = function () {
	playsound("/resources/audio/tada.mp3"); // 게임에 승리하였을 알리는 빵빠레
audio.onended = null;
	}
	}
	else
...
playsound("/resources/audio/bad.mp3"); // 오답을 선택한 경우 울리는 경고음
failNum++;
